        ...
        for token_id, life in action.token_life {
            if life {
                ...
                if ok {
                    ...
                    t := make_token(token_id, {{u32(action.token_history[token_id].x), u32(action.token_history[token_id].y)}, {0, 0}}, action.color, "", action.token_initiative_history[token_id].x)
